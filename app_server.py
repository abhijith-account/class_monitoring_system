...
try:
    ...
    print("FATAL ERROR: classroom_monitor.py not found.")
    sys.exit(1)
...
def generate_frames():
    ...
    while True:
        if not is_running:
            # If stopped, send one offline frame and wait
...
def background_monitor_task():
    ...
    print("[SYSTEM] Connecting to Camera (Attempt 1)...")
    ...
    while not camera.status: 
        if time.time() - start_cam_time > 15: 
            print("[WARNING] Standard Camera Timed Out. Trying Pi Camera Mode...")
            camera.release()
            # Fallback to Pi Camera Mode
            camera = ThreadedCamera(src=0, use_pi_camera=True)
            time.sleep(3)
            if not camera.status:
                print("[FATAL ERROR] Could not initialize ANY camera.")
                is_running = False
                return
        time.sleep(0.5)
...
# [NEW] Route to handle the Authorization Switch
...
def update_policy():
    ...
    try:
        ...
        # "authorized" will be True (Switch ON) or False (Switch OFF)
        authorized_status = data.get("authorized", False)
        
        if monitor:
            monitor.set_mobile_policy(authorized_status)
            return jsonify({"status": "success", "mode": "Authorized" if authorized_status else "Restricted"})
        else:
            return jsonify({"status": "error", "message": "Monitor not started"})
